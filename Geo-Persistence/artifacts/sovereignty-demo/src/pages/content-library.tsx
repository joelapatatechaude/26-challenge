...
import {
  ...
  HelpCircle, Loader2, Send, Globe, FileBadge, ChevronDown,
  RefreshCcw, Pin, MoreHorizontal, FileCheck2, Languages
} from "lucide-react";
import { OUTPUT_LANGS, type OutputLang } from "./ai-toolkit";
import { streamPptChat } from "@workspace/api-client-react";

type Source = { id: string; name: string; type: "pdf" | "url" | "doc" | "slide"; region: string; size: string; pinned?: boolean };
type NoteCard = { id: string; title: string; body: string; type: "summary" | "guide" | "faq" | "brief" | "outline" };
...
const NOTE_CARDS: NoteCard[] = [
  { id: "n1", type: "summary", title: "Executive Summary", body: "Red Hat's Digital Sovereignty portfolio centers on three pillars: data residency (OpenShift on sovereign infrastructure), operational independence (disconnected/air-gapped deployments), and regulatory compliance (GDPR, NIS2, DSGVO, EU Data Act). Key differentiators vs. hyperscalers: open-source auditability, no vendor lock-in, and certified sovereign cloud partnerships across EMEA." },
  { id: "n2", type: "guide", title: "Field Study Guide", body: "**Key talking points:** (1) Data never leaves customer jurisdiction. (2) OpenShift supports air-gapped & disconnected installations. (3) Certified partners: T-Systems OTC, OVHcloud, Outscale. (4) EU Data Act compliance roadmap included. **Common objections:** 'We use hyperscaler sovereign zones' → Address: contractual guarantees vs. technical isolation." },
  { id: "n3", type: "faq", title: "Customer FAQ", body: "**Q: Does Red Hat store any customer data?** A: No — OpenShift is deployed within the customer's own sovereign infrastructure. **Q: Is OpenShift certified for DSGVO?** A: Yes — BSI C5 attestation available for German public sector. **Q: Can we get a sovereign cloud deployment with an EU-based partner?** A: Yes — T-Systems OTC, OVHcloud SecNumCloud, and Ionos are certified partners." },
  { id: "n4", type: "brief", title: "Competitive Briefing", body: "VMware (Broadcom): Post-acquisition pricing concerns create displacement opportunity. Azure/AWS Sovereign: Contractual promises only — data still processed on hyperscaler infrastructure. Google Sovereign: Limited EMEA partner availability. **Red Hat edge:** full open-source stack, customer-controlled encryption keys, certified on-prem sovereign cloud partners with legal guarantees." },
];
...
const RH_PRODUCTS = [
  { name: "OpenShift", color: "#EE0000" }, { name: "RHEL", color: "#CC0000" },
  { name: "Ansible", color: "#0066CC" }, { name: "ACM", color: "#3E8635" },
  { name: "ROSA", color: "#EC7A08" }, { name: "OpenShift AI", color: "#7B4FC1" },
  { name: "Quay", color: "#0066CC" }, { name: "Satellite", color: "#3E8635" },
];
...
const noteTypeLabel: Record<NoteCard["type"], string> = {
  summary: "Summary", guide: "Study Guide", faq: "FAQ", brief: "Briefing Doc", outline: "Outline",
};
...
export default function ContentLibrary() {
  ...
  const langMeta = OUTPUT_LANGS.find(l => l.value === responseLang)!;

  useEffect(() => { chatEndRef.current?.scrollIntoView({ behavior: "smooth" }); }, [messages, thinking]);

  const toggleSource = (id: string) => setSelectedSources(prev => {
  ...
  const filteredSources = SOURCES.filter(s =>
    s.name.toLowerCase().includes(searchQ.toLowerCase()) || s.region.toLowerCase().includes(searchQ.toLowerCase())
  );
  ...
  const handleSend = async (text?: string) => {
    ...
    try {
      for await (const chunk of streamPptChat(msg, {
        ...
        language: responseLang !== "en" ? responseLang : null,
      })) {
  ...
  const handleGenerateNote = (type: NoteCard["type"], label: string) => {
    ...
    setTimeout(() => {
      ...
      const newNote: NoteCard = {
        id: "n" + Date.now(), type, title: label + " — EMEA Territory",
        ...
        <aside className="w-64 shrink-0 border-r border-[var(--rh-charcoal-light)] flex flex-col bg-[var(--rh-charcoal-mid)]">
          <div className="px-4 pt-4 pb-3 border-b border-[var(--rh-charcoal-light)]">
            ...
            <div className="relative">
              ...
              <input value={searchQ} onChange={e => setSearchQ(e.target.value)} placeholder="Filter sources..." className="w-full bg-[var(--rh-charcoal)] border border-[var(--rh-charcoal-light)] rounded pl-7 pr-3 py-1.5 text-xs focus:outline-none focus:border-[var(--rh-red)] text-white placeholder:text-[var(--rh-silver)]/50 transition-colors" />
            </div>
          </div>
          <div className="flex-1 overflow-y-auto py-2 px-2">
            {filteredSources.map(s => (
              <button key={s.id} onClick={() => toggleSource(s.id)} className={`w-full text-left flex items-start gap-2.5 px-2 py-2.5 rounded-md mb-1 transition-all ${selectedSources.has(s.id) ? "bg-[var(--rh-charcoal-light)] text-white" : "text-[var(--rh-silver)] hover:bg-[var(--rh-charcoal-light)]/50"}`}>
                <div className={`w-4 h-4 mt-0.5 rounded border flex items-center justify-center shrink-0 transition-all ${selectedSources.has(s.id) ? "bg-[var(--rh-red)] border-[var(--rh-red)]" : "border-[var(--rh-charcoal-light)]"}`}>
                  {selectedSources.has(s.id) && <div className="w-2 h-2 bg-white rounded-sm" />}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1 mb-0.5">{typeIcon(s.type)}{s.pinned && <Pin className="w-2.5 h-2.5 text-[var(--rh-orange)]" />}</div>
          ...
          <div className="p-3 border-t border-[var(--rh-charcoal-light)]">
            ...
            <div className="flex gap-2 mt-2">
              <button className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded text-[10px] text-[var(--rh-silver)] hover:text-white hover:bg-[var(--rh-charcoal-light)] transition-all"><Link2 className="w-3 h-3" /> URL</button>
              <button className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded text-[10px] text-[var(--rh-silver)] hover:text-white hover:bg-[var(--rh-charcoal-light)] transition-all"><Upload className="w-3 h-3" /> File</button>
            </div>
          </div>
        </aside>
          ...
          <div className="px-6 py-3 border-b border-[var(--rh-charcoal-light)] shrink-0">
            <div className="flex items-center justify-between mb-2.5">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1.5 px-2 py-1.5 rounded-md border border-[var(--rh-charcoal-light)] bg-[var(--rh-charcoal)]">
                  <Languages className="w-3 h-3 text-[var(--rh-silver)]" />
                  <select
                    value={responseLang}
                    onChange={e => setResponseLang(e.target.value as OutputLang)}
                    className="bg-transparent text-[11px] text-white border-0 focus:outline-none cursor-pointer"
                ...
                {responseLang !== "en" && (
                  <span className="text-[10px] text-[var(--rh-blue)] border border-[var(--rh-blue)]/30 rounded px-1.5 py-0.5">
                    {langMeta.flag} {langMeta.regulatory}
                  </span>
                )}
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {RH_PRODUCTS.map(p => (
                <span key={p.name} className="text-[9px] font-semibold px-2 py-0.5 rounded-full border" style={{ color: p.color, borderColor: p.color + "40", backgroundColor: p.color + "12" }}>{p.name}</span>
              ))}
            </div>
          </div>
            ...
            {messages.length <= 1 && (
              <div className="mb-2">
                ...
                <div className="flex flex-col gap-1.5">
                  {SUGGESTIONS.map((s, i) => (
                    <button key={i} onClick={() => handleSend(s)} className="text-left flex items-center gap-2 px-3 py-2 rounded-md bg-[var(--rh-charcoal-mid)] border border-[var(--rh-charcoal-light)] text-[11px] text-[var(--rh-silver)] hover:text-white hover:border-[var(--rh-red)]/50 transition-all group">
        ...
        <aside className="w-72 shrink-0 flex flex-col bg-[var(--rh-charcoal-mid)]">
          <div className="px-4 pt-4 pb-3 border-b border-[var(--rh-charcoal-light)]">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {([["summary", "Summary", AlignLeft], ["guide", "Study Guide", BookOpen], ["faq", "FAQ", HelpCircle], ["brief", "Briefing Doc", ListChecks]] as const).map(([type, label, Icon]) => (
                <button key={type} onClick={() => handleGenerateNote(type, label)} disabled={generating} className="flex items-center gap-1.5 px-2 py-2 rounded-md bg-[var(--rh-charcoal)] border border-[var(--rh-charcoal-light)] text-[10px] text-[var(--rh-silver)] hover:text-white hover:border-[var(--rh-red)]/50 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
                  {generating && generatingType === label ? <Loader2 className="w-3 h-3 text-[var(--rh-red)] animate-spin" /> : <Icon className="w-3 h-3 text-[var(--rh-red)]" />}
                  {label}
                </button>
              ))}
            </div>
            ...
            {activeNote ? (
              <div className="p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button className="p-1 rounded hover:bg-[var(--rh-charcoal-light)] transition-colors"><MoreHorizontal className="w-3.5 h-3.5 text-[var(--rh-silver)]" /></button>
                </div>
                <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-semibold uppercase tracking-wider mb-2 ${noteTypeBadge[activeNote.type]}`}>
                  {noteTypeIcon(activeNote.type)}{noteTypeLabel[activeNote.type]}
                </div>
              ...
              <div className="p-3 space-y-2">
                {notes.map(note => (
                  <button key={note.id} onClick={() => setActiveNote(note)} className="w-full text-left p-3 rounded-md bg-[var(--rh-charcoal)] border border-[var(--rh-charcoal-light)] hover:border-[var(--rh-red)]/40 hover:bg-[var(--rh-charcoal-light)]/30 transition-all group">
                    <div className="flex items-start justify-between gap-2 mb-1.5">
                      <div className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-semibold uppercase tracking-wider ${noteTypeBadge[note.type]}`}>
                        {noteTypeIcon(note.type)}{noteTypeLabel[note.type]}
                      </div>
